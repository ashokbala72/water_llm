...
def load_balance_tanks(location='London'):
    ...
    per_tank_fills = sensors.get('telemetry', {}).get('per_tank_fill', {})  # Optional per-tank fill
...
def storm_response_coordinator(location='London'):
    ...
    return {'location': location, 'inputs': inputs, 'overflow_predicted': overflow,
            'asset_status': asset_status, 'risk_level': risk, 'control_advice': control, 'control_result': control_result, 'alert_sent': alert, 'anomalies': anomalies, 'compliance_status': compliance, 'regulatory_report': report, 'genai_advisory': advisory, 'infra_upgrades': upgrades, 'tank_balancing': tank_balancing}
