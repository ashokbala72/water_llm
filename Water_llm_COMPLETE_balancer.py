...
def get_real_time_inputs(location='London'):
    ...
    return {'location': location, 'rainfall_mm': weather.get('rainfall_mm', 0), 'inflow_rate_lps': sensors.get('inflow_rate_lps', 0), 'tank_fill_percent': sensors.get('tank_fill_percent', 0), 'timestamp': weather.get('timestamp', '')}
...
def fetch_tank_config(location='London'):
    ...
    try:
        ...
        logger.error(f'❌ Failed to read tank_config.csv: {e}')
        return []

def load_balance_tanks(location='London'):
    """Load balance tanks function."""
...
def storm_response_coordinator(location='London'):
    ...
    return {'location': location, 'inputs': inputs, 'overflow_predicted': overflow, 'risk_level': risk, 'control_advice': control, 'control_result': control_result, 'alert_sent': alert, 'anomalies': anomalies, 'compliance_status': compliance, 'regulatory_report': report, 'genai_advisory': advisory, 'infra_upgrades': upgrades, 'tank_balancing': tank_balancing}